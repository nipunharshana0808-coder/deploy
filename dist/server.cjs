...
var oncologyOptions = [
  ...
  "Waldenstr\xF6m macroglobulinemia",
  ...
  "Pheochromocytoma",
  "Parathyroid",
  "Carcinoid / Neuroendocrine (general)",
...
app.put("/api/patients/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const record = { ...req.body, id, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
    record.driveFolderId = await ensureDriveFolder(record);
    res.json(await saveDocument("patients", id, record));
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.get("/api/patients/trash", async (_req, res) => {
  try {
    ...
    deletedPatients.sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || "")));
