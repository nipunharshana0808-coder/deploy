...
import { deleteCollectionRecord, getCollectionRecord, readCollectionStore, upsertCollectionRecord } from "./local-store.js";

let firebaseProjectId = process.env.FIREBASE_WEB_PROJECT_ID || process.env.VITE_FIREBASE_PROJECT_ID;
...
function warnLocalFallback(reason: unknown) {
  if (warnedLocalFallback) return;
  warnedLocalFallback = true;
  console.warn("Using local JSON datastore fallback for Firestore operations:", reason);
}
...
export async function getFirestoreDoc(collection: string, id: string): Promise<Record<string, any> | null> {
  try {
    ...
    return getCollectionRecord(collection, id);
  }
}
...
export async function listCollection(collection: string): Promise<Record<string, any>[]> {
  try {
    ...
    warnLocalFallback(error);
    return readCollectionStore(collection);
  }
}
...
export async function saveDocument(collection: string, id: string, data: Record<string, any>) {
  try {
    ...
    warnLocalFallback(error);
    return upsertCollectionRecord(collection, id, { ...data, id });
  }
}
