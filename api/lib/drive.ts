...
function isOfflineFallbackError(error: unknown) {
  ...
  return /connect|fetch failed|oauth|offline|ECONN|EPERM|ENOTFOUND|Google auth failed/i.test(message);
}

function syntheticId(prefix: string, seed: string) {
  return `${prefix}_${Buffer.from(seed).toString("hex").slice(0, 16)}`;
}
...
export async function ensureDriveFolder(patient: any) {
  ...
  try {
    ...
    const folderId = patient.driveFolderId || syntheticId("local_folder", patient.id || JSON.stringify(patient));
    if (patient.id) {
      try {
        await saveDocument("patients", patient.id, { ...patient, driveFolderId: folderId, updatedAt: new Date().toISOString() });
      } catch (persistError) {
        console.warn(`Could not persist fallback driveFolderId for patient ${patient.id}:`, persistError);
      }
    }
    return folderId;
  }
...
export async function uploadToDrive(payload: any, folderId: string) {
  try {
    ...
    if (!isOfflineFallbackError(error)) throw error;
    const seed = `${folderId}:${payload.name}:${Date.now()}`;
    return {
      id: syntheticId("local_file", seed),
      name: payload.name,
      mimeType: payload.mimeType || "application/octet-stream",
...
export async function recursivelyDeleteDriveFolder(folderId: string, maxRetries = 3) {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (isOfflineFallbackError(e)) return;
      retryCount++;
