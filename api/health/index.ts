...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === "GET") {
    ...
    let firebaseCredentialValid = false;
    if (firebaseServiceAccountRaw) {
      try {
        firebaseServiceAccount = JSON.parse(firebaseServiceAccountRaw);
        crypto.createPrivateKey((firebaseServiceAccount as any).private_key);
        firebaseCredentialValid = Boolean((firebaseServiceAccount as any).client_email);
      } catch {}
    }

    return res.json({
      ok: true,
      firestore: Boolean(firebaseProjectId && firebaseCredentialValid),
