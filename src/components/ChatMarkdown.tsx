  ...
  while ((m = INLINE_TOKEN_RE.exec(text)) !== null) {
    ...
    if (tok.startsWith("**")) {
      ...
      out.push(
        <code
          key={`${keyPrefix}-c-${i++}`}
          className="px-1.5 py-0.5 mx-0.5 rounded-md bg-slate-100 dark:bg-slate-800/80 text-[0.85em] text-[#7A8C70] dark:text-emerald-300 border border-slate-200 dark:border-slate-700"
        >
          {tok.slice(1, -1)}
        </code>
      );
      ...
      if (linkM) {
        out.push(
          <a
            ...
            className="inline-flex items-center gap-0.5 font-semibold text-[#7A8C70] dark:text-emerald-300 underline decoration-dotted underline-offset-2 hover:text-[#68785E]"
          >
            {linkM[1]}
...
const sectionPalette: Record<SectionKind, { bar: string; chip: string; icon: React.ComponentType<{ className?: string }>; label: string }> = {
  ...
  default: {
    bar: "bg-gradient-to-r from-[#7A8C70] via-[#9CAA92] to-[#7A8C70]",
    chip: "bg-[#7A8C70]/10 text-[#4A5444] border-[#7A8C70]/30 dark:bg-[#7A8C70]/20 dark:text-emerald-200 dark:border-[#7A8C70]/40",
    icon: ClipboardList,
    label: "Note",
  },
};
  ...
  switch (block.kind) {
    ...
    case "h2": {
      ...
      return (
        ...
        >
          <div className="bg-white/50 dark:bg-slate-800/30 rounded-2xl p-3.5 border border-slate-200/60 dark:border-slate-700/40 shadow-sm">
    ...
    case "h3":
      return (
        <h3
          key={idx}
          style={delay}
          className={`${fadeUp} text-[13px] font-bold text-slate-800 dark:text-slate-100 mt-3 mb-1.5 px-1 border-l-2 border-[#7A8C70]/60 pl-2.5`}
        >
          {block.text}
        </h3>
      );
      ...
      return (
        ...
        >
          {block.items.map((it, j) => (
            <li key={j} className="flex gap-2 text-[12.5px] leading-relaxed text-slate-700 dark:text-slate-200 py-1 px-2 rounded-lg bg-white/30 dark:bg-slate-800/30 hover:bg-white/50 dark:hover:bg-slate-800/50 transition-colors">
              <ChevronRight className="h-3.5 w-3.5 mt-0.5 flex-shrink-0 text-[#7A8C70] dark:text-emerald-400" />
    ...
    case "ol":
      return (
        ...
        >
          {block.items.map((it, j) => (
            <li key={j} className="flex gap-2 text-[12.5px] leading-relaxed text-slate-700 dark:text-slate-200 py-1 px-2 rounded-lg bg-white/30 dark:bg-slate-800/30 hover:bg-white/50 dark:hover:bg-slate-800/50 transition-colors">
              <span className="flex-shrink-0 h-5 w-5 rounded-full bg-[#7A8C70]/15 dark:bg-emerald-500/15 text-[#7A8C70] dark:text-emerald-300 text-[10px] font-bold flex items-center justify-center border border-[#7A8C70]/30 dark:border-emerald-500/30">
                {j + 1}
              </span>
    ...
    case "quote":
      return (
        <blockquote
          key={idx}
          style={delay}
          className={`${fadeUp} my-2 pl-3 border-l-2 border-[#7A8C70]/60 italic text-[12.5px] text-slate-600 dark:text-slate-300`}
        >
