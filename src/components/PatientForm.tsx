...
export default function PatientForm({ 
  ...
  const autoId = initialData?.auto_id || existingPatientId ? '' : `onco-${String(Date.now()).slice(-4)}`;
  
  useEffect(() => {
    if (!initialData && !existingPatientId) {
      setFormState(prev => ({ ...prev, auto_id: autoId }));
    }
  }, [autoId, initialData, existingPatientId]);
  ...
  // Load existing patient data from API
  useEffect(() => {
    if (existingPatientId) {
      setLoadingExisting(true);
      setExistingLoadError("");
      fetch(`/api/patients/${existingPatientId}`)
        .then(res => {
        ...
        .catch(err => {
          setExistingLoadError(err.message);
          setLoadingExisting(false);
        });
    }
  }, [existingPatientId]);
  ...
  const ensurePatientSavedBeforeUpload = async () => {
    ...
    try {
      ...
      const endpoint = isUpdate ? `/api/patients/${formState.id}` : "/api/patients";
      ...
      if (!res.ok) {
        ...
        throw new Error(errData?.error || "Failed to save patient");
      }
  ...
  const processAttachment = async (file: File, isPureMedia = false) => {
    ...
    try {
      ...
      if (uploadPatientId && !alreadyUploaded) {
        try {
          if (isPureMedia) {
            setVaultProgress(72);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveSuccess(true);
      await notify("Patient clinical record saved in secure repository.", "Record Saved", "success");
      setTimeout(() => onSaveSuccess(), 700);
    } catch (err: any) {
      await notify(err.message || "Error saving record. Please try again.", "Save Failed", "danger");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (existingLoadError) {
    return (
      <div className="p-8 text-center bg-rose-500/10 border border-rose-500/30 text-rose-500 rounded-xl space-y-4">
        ...
        <p className="font-semibold">{existingLoadError}</p>
        <button onClick={onCancel} className="px-4 py-2 bg-slate-800 rounded font-semibold text-xs text-white">
          Go Back
        </button>
      </div>
    );
