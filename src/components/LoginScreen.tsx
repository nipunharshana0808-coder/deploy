...
export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
    ...
    try {
      ...
      if (result) {
        ...
        onLoginSuccess({
          ...
          email,
          role,
          avatarColor: role === "admin" ? "bg-[#7A8C70]" : "bg-[#A98467]"
        });
      }
    } catch (err: any) {
  ...
  if (showAgreementView) {
    return (
      <div className="min-h-screen bg-[#F7F6F2] dark:bg-[#12140F] flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors duration-200 relative overflow-hidden page-fade-in">
        ...
        <div className="sm:mx-auto sm:w-full sm:max-w-2xl relative z-10 px-4 sm:px-0">
          <div className="glass-card rounded-3xl overflow-hidden shadow-2xl relative">
            ...
            <div className="px-6 py-5 border-b border-natural-border/40 flex items-center justify-between bg-white/10 dark:bg-slate-900/10 backdrop-blur-md">
              ...
              <button 
                ...
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-150 dark:hover:bg-slate-800 transition-colors"
                title="Back to login"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="pt-6 border-t border-natural-border/30 flex flex-col sm:flex-row items-center gap-3">
                <button
                  ...
                  className="w-full sm:w-auto px-5 py-2.5 bg-[#5B6B52] dark:bg-[#7A8C70] hover:bg-[#4A5444] dark:hover:bg-[#68785E] text-white rounded-xl text-xs font-bold shadow-md transition-colors cursor-pointer text-center"
                >
                  I Confirm & Accept
                </button>
                <button
                  type="button"
                  onClick={() => setShowAgreementView(false)}
                  className="w-full sm:w-auto px-5 py-2.5 border border-[#5B6B52] dark:border-slate-700 hover:bg-white/10 dark:hover:bg-slate-800 text-[#5B6B52] dark:text-slate-200 rounded-xl text-xs font-bold transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                >
    ...
    <div className="min-h-screen bg-[#F7F6F2] dark:bg-[#12140F] flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors duration-200 relative overflow-hidden page-fade-in">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="flex justify-center">
          <div className="h-16 w-16 bg-[#7A8C70] rounded-2xl flex items-center justify-center text-white shadow-md border border-white/10 backdrop-blur-md">
            <Activity className="h-10 w-10 animate-pulse" />
          </div>
        </div>
        <h2 className="mt-6 text-center text-3xl font-bold tracking-tight text-[#4A5444] dark:text-white">
          AI DMS
        </h2>
        <p className="mt-2 text-center text-sm text-[#4A5444] font-semibold dark:text-slate-300">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10 px-4 sm:px-0">
        <div className="glass-card py-8 px-6 sm:px-10 rounded-3xl">
          ...
          {/* Status Indicators */}
          <div className="mb-6 bg-[#7A8C70]/10 dark:bg-[#7A8C70]/20 border border-[#7A8C70]/30 rounded-2xl p-4 flex gap-3 text-sm text-[#2C3527] dark:text-[#E0DDD2] backdrop-blur-xs">
            <ShieldCheck className="h-5 w-5 text-[#5B6B52] dark:text-[#7A8C70] flex-shrink-0 animate-bounce" />
            <div>
              <p className="font-bold text-xs text-[#2C3527] dark:text-slate-100">Medical Security Protocol Active</p>
              <p className="text-[10px] text-[#4A5444] dark:text-slate-300 font-semibold mt-0.5">Dual end-to-end data validation. Compliance secured.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4 animate-fade-in">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 bg-white/40 dark:bg-slate-900/30 border border-slate-400 dark:border-slate-700 rounded-xl text-xs focus:ring-1 focus:ring-[#7A8C70] focus:border-[#7A8C70] outline-none text-slate-800 dark:text-white"
              />
            </div>
            ...
            <AnimatedButton
              ...
              size="lg"
              fullWidth
              className="bg-[#5B6B52] dark:bg-[#7A8C70] hover:bg-[#4A5444] dark:hover:bg-[#68785E] border-transparent"
            >
            ...
            <button
              ...
              disabled={isLoggingIn}
              className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-slate-300 dark:border-slate-700 rounded-xl shadow-sm text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 active:scale-[0.98] transition-all cursor-pointer"
            >
               Sign in with Google
            </button>
              ...
              <div className="flex flex-col items-center gap-2">
                {!agreed && (
                  <button
                    ...
                    className="px-4 py-1.5 bg-[#5B6B52] dark:bg-[#7A8C70] text-white rounded-lg text-xs font-bold hover:bg-[#4A5444] dark:hover:bg-[#68785E] transition-colors"
                  >
                    I Agree to Terms
                  </button>
                )}
                ...
                <button
                  ...
                  className="text-[#5B6B52] dark:text-[#9FB195] hover:text-[#3B4734] dark:hover:text-[#C3CFBD] font-semibold underline cursor-pointer focus:outline-none relative z-50 text-[10px]"
                >
                  View User Agreement
                </button>
                  
                {agreed && (
                  <span className="text-[#5B6B52] dark:text-[#9FB195] font-bold text-[10.5px]">
