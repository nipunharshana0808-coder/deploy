...
export default function PatientView({
  ...
  const normalizedPatient = patient ? useMemo(() => normalizePatientData(patient), [patient]) : null;
  ...
  // Initialize chat when patient loads
  useEffect(() => {
    if (normalizedPatient && chatThread.length === 0) {
      setChatThread([
        { 
          sender: "ai", 
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          patientRecord: normalizedPatient || patient,
          query: userText
        })
      });
  ...
  if (fetchError || !patient) {
    return (
      <div className="p-8 text-center bg-rose-500/10 border border-rose-500/30 text-rose-500 rounded-xl space-y-4">
        <ShieldAlert className="w-8 h-8 mx-auto" />
        <p className="font-semibold">{fetchError || "Failed to load patient record."}</p>
        <button onClick={onBack} className="px-4 py-2 bg-slate-800 rounded font-semibold text-xs text-white">
          Go Back
        </button>
      </div>
    );
