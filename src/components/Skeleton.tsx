...
export const SkeletonCard: React.FC<{ className?: string; dark?: boolean }> = ({ className = "", dark }) => (
  <div className={`border border-[#D9D5CB] dark:border-slate-700/80 rounded-2xl p-5 bg-white/60 dark:bg-slate-800/30 space-y-3 ${className}`}>
...
export const SkeletonTable: React.FC<{ rows?: number; cols?: number; className?: string; dark?: boolean }> = ({
  ...
  <div className={`border border-[#D9D5CB] dark:border-slate-700/80 rounded-xl overflow-hidden ${className}`}>
    <div className="grid grid-cols-12 gap-2 p-3 bg-slate-50 dark:bg-slate-800/40 border-b border-[#D9D5CB] dark:border-slate-700">
      {Array.from({ length: cols }).map((_, i) => (
        <Skeleton key={i} className="h-3 col-span-3" dark={dark} />
      ))}
    </div>
    {Array.from({ length: rows }).map((_, r) => (
      <div key={r} className="grid grid-cols-12 gap-2 p-3 border-b border-[#D9D5CB]/50 dark:border-slate-800/60 last:border-b-0">
...
export const SkeletonPatientCard: React.FC<{ dark?: boolean }> = ({ dark }) => (
  <div className="border border-[#D9D5CB] dark:border-slate-700/80 rounded-2xl p-5 bg-white/80 dark:bg-slate-800/40 space-y-3 medical-card">
    <div className="flex items-center gap-3">
