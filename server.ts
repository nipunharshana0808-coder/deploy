...
const primaryGeminiModel = env("GEMINI_MODEL_PRIMARY") || "gemini-2.0-flash-lite";
const secondaryGeminiModel = env("GEMINI_MODEL_SECONDARY") || "gemini-2.0-flash-lite";

function parseServiceAccount(raw: string) {
  const readFromLocalEnvNew = () => {
    try {
      const file = fs.readFileSync(path.join(process.cwd(), ".ene.new"), "utf8");
...
const oncologyOptions = [
  // Head & Neck
  "Oral cavity", "Oropharynx", "Nasopharynx", "Hypopharynx", "Larynx",
  "Lip", "Tongue", "Parotid", "Submandibular", "Salivary gland (other)",
  "Mandibular", "Maxillary sinus", "Nasal cavity / Paranasal sinuses",
  "Thyroid", "Eye / Orbital", "Ocular melanoma", "Retinoblastoma",
  "Lacrimal gland", "Ear / Temporal bone",
  // Thoracic / Respiratory
  "Lung", "Mesothelioma", "Thymus / Thymoma", "Chest wall",
  // Breast
  "Breast", "Male breast cancer",
  // Gastrointestinal
  "Esophagus", "Gastric / Stomach", "Small bowel", "Large bowel / Colon",
  "Rectal", "Anal canal", "Appendix", "Liver / Hepatocellular",
  "Gall bladder", "Biliary tree / Cholangiocarcinoma", "Pancreas",
  "GIST", "GI Neuroendocrine tumor",
  // Genitourinary
  "Renal / Kidney", "Bladder", "Prostate", "Penis", "Testicular",
  "Urethra", "Ureter",
  // Gynecologic
  "Ovary", "Uterus / Cervix", "Endometrial", "Fallopian tubes",
  "Vaginal", "Vulvar", "Gestational trophoblastic disease",
  // Musculoskeletal / Sarcoma
  "Bone / Osteosarcoma", "Chondrosarcoma", "Ewing sarcoma",
  "Soft tissue sarcoma", "Leiomyosarcoma", "Liposarcoma",
  "Rhabdomyosarcoma", "Synovial sarcoma", "Kaposi sarcoma",
  "Dermatofibrosarcoma protuberans",
  // Skin
  "Skin / Melanoma", "Skin / Squamous cell", "Skin / Basal cell",
  "Merkel cell carcinoma",
  // Hematologic / Lymphoid
  "Leukemia / AML", "Leukemia / ALL", "Leukemia / CLL", "Leukemia / CML",
  "Lymphoma / Hodgkin", "Lymphoma / Non-Hodgkin",
  "Multiple myeloma / Plasma cell", "Myelodysplastic syndrome (MDS)",
  "Myeloproliferative neoplasm (MPN)", "Waldenström macroglobulinemia",
  // Central Nervous System
  "Brain / Glioma", "Glioblastoma", "Meningioma", "Medulloblastoma",
  "Pituitary / Sellar", "Spinal cord tumors", "Primary CNS lymphoma",
  "Neuroblastoma",
  // Endocrine
  "Adrenals", "Adrenal cortical carcinoma", "Pheochromocytoma",
  "Parathyroid", "Carcinoid / Neuroendocrine (general)",
  "Multiple endocrine neoplasia",
  // Pediatric / Embryonal
  "Wilms tumor / Nephroblastoma", "Hepatoblastoma",
  "Langerhans cell histiocytosis", "Germ cell tumors",
  // Other
  "Spleen", "Heart", "Peritoneal", "Retroperitoneal",
  "Cancer of unknown primary (CUP)", "Other",
];
...
function normalizeExtraction(data: Record<string, any>) {
  ...
  data.oncology_types = Array.from(new Set([...matchedTypes, ...rawOncologyTypes.filter((item) => oncologyOptions.includes(item))]));
...
app.get("/api/files", async (_req, res) => {
  try {
    res.json(await listCollection("files"));
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
- Demographics: first_name, last_name, initials, dob, age, gender, nic, tp, oncology_types, oncology_type, geographic_accessibility -> use these.
...
- Examination findings (examFindingsTable): per-system clinical exam findings grouped by date. Allowed organ_system values: General Appearance, ECOG / Performance Status, BP, Pulse (Rate & Rhythm), SpO2, Temperature, Respiratory Rate, Head & Neck, Eyes / Ophthalmic, Ears, Nose & Sinuses, Oral Cavity & Throat, Lymph Nodes, RS (Respiratory), Breast Exam, CVS (Cardiovascular), Peripheral Vascular, Varicose Veins, P/A (Per Abdomen), Groin & Hernia, Rectal / DRE, CNS (Neurological), Cranial Nerves, Cerebellar Examination, Sensory Examination, Upper Limbs, Lower Limbs, Radial Nerve, Median Nerve, Axillary Nerve, Carpal Tunnel, Sciatic Nerve, Common Peroneal Nerve, MSK (Musculoskeletal), Knee, Hip, Spine, TMJ, Skin, Local / Lesion Exam, Lumps & Bumps. Each row has date + organ_system + findings + notes. Multiple organ systems can share the same date. USE examFindingsTable first; only fall back to top-level exam_findings / systemic_exam scalars for unstructured prose.\n- Anthropometric measures (otherAnthropometricTable): one row per measure per date (date + measure + value + unit). Multiple measures can share the same date (e.g. same visit: waist circumference + MUAC + skinfold thickness). measure dropdown values: Waist circumference, Hip circumference, Mid-upper arm circumference (MUAC), Head circumference, Neck circumference, Chest circumference, Thigh circumference, Calf circumference, Skinfold thickness (triceps), Skinfold thickness (subscapular), Skinfold thickness (suprailiac), Arm span, Sitting height, Waist-to-hip ratio. Unit dropdown: cm, mm, kg, m, in, ft.
...
- Oncological outcomes: oncologicalOutcomeTable. Emit one row per dated assessment. Map response criteria and response findings, progression/recurrence dates and sites, survival status/date, cause of death, follow-up ECOG, tumour-marker follow-up, imaging follow-up, and outcome notes here. Do not place these oncology follow-up outcomes in the surgical outcome table.
