...
import { adaptPatientForNewUi } from "./api/lib/patient-schema.js";

dotenv.config({ path: ".env.clean" });
dotenv.config({ path: ".ene.new" });
dotenv.config();

// Keep older local .ene.new files compatible with the Vite client variable names.
const clientEnvAliases: Record<string, string> = {
...
let firebaseProjectId = env("FIREBASE_WEB_PROJECT_ID", "VITE_FIREBASE_PROJECT_ID");
...
let firebaseCredentialError = "";

if (firebaseServiceAccount?.private_key) {
  try {
    crypto.createPrivateKey(firebaseServiceAccount.private_key);
  } catch {
    firebaseCredentialError =
      "FIREBASE_SERVICE_ACCOUNT_JSON contains an invalid or placeholder private_key. " +
      "Download a real Firebase Admin SDK service-account JSON file and replace the complete environment variable value.";
  }
}
...
app.get("/api/health", (_req, res) => {
  res.json({
    ...
    configurationError: firebaseCredentialError || undefined,
  });
});
...
// Trash listing must appear BEFORE /api/patients/:id to avoid Express matching "trash" as an :id param
app.get("/api/patients/trash", async (_req, res) => {
  try {
    const patients = await listCollection("patients");
    const trashed = patients.filter((p: any) => p.isDeleted);
    trashed.sort((a: any, b: any) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || "")));
    res.json(trashed);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/api/files", async (req, res) => {
  try {
    ...
    res.json(patientId ? files.filter((file: any) => file.patientId === patientId) : files);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
