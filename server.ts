...
const primaryGeminiModel = env("GEMINI_MODEL_PRIMARY") || "gemini-2.5-flash";
const secondaryGeminiModel = env("GEMINI_MODEL_SECONDARY") || primaryGeminiModel;
  ...
  for (const attempt of attempts) {
    try {
      ...
      const response = await ai.models.generateContent({
        model: attempt.model === "auto" ? "gemini-2.5-flash" : attempt.model,
        contents,
...
- Use short, scannable structure: bold the heading, then 1-3 sentence bullets. Use markdown for headings/lists/bold.
